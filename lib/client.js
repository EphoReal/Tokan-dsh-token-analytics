  ...
  factory: (require) => {
    ...
    function SessionSelector() {
      ...
      var options = sessionList.map(function (s) {
        ...
        return h('option', { key: s.sessionId, value: s.sessionId, title: title, style: { color: 'var(--dsw-alias-label-primary, #e5e7eb)', background: '#000' } }, label);
      });
    ...
    function VisualCharts(props) {
      ...
      var toolInfoContent = [
        { name: '展示内容 / What This Shows', desc: '按操作类型统计 Token 消耗，每根柱条代表该类操作的总 Token 用量。/ Token consumption breakdown by operation type. Each bar represents the total tokens used by that category of operations.' },
        { name: '为何重要 / Why It Matters', desc: '了解哪些操作消耗最多 Token，有助于发现潜在的优化机会。/ Understanding which operations consume the most tokens helps identify potential optimization opportunities.' },
        { name: '如何阅读 / How to Read', desc: '柱条越长消耗越多；百分比是该操作占全会话 Token 的比例。/ Longer bars mean more tokens consumed; the percentage shows the share of total session tokens.' },
      ];
      ...
      function ToolInfoButton() {
        ...
        return h('div', {
          ...
          toolInfoVisible ? h('div', {
            ...
            h('div', { style: { fontSize: '11px', fontWeight: 600, color: 'var(--dsw-alias-label-primary, #1f2937)', marginBottom: '6px' } }, '工具参考 / Tool Reference'),
            toolInfoContent.map(function(info, idx) {
              return h('div', { key: idx, style: { marginBottom: '6px', paddingBottom: '4px', borderBottom: idx < toolInfoContent.length - 1 ? '1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.1))' : 'none' } },
